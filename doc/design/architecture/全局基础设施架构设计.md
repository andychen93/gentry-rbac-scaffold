...
>
> **⚠️ 部分内容已过期（2026-09-01）**：本仓库已拿掉多租户机制，本文中所有「多租户拦截器 /
> `GentryTenantManager` / `TenantEntity` / `tenant_id` 自动追加 / `@IgnoreTenant`」相关描述
> 已不适用（组件已删除，当前是 9 个 P0/P1/P2 组件）。其余章节（异常处理、自动填充、数据权限、
> 限流、追踪等）仍然有效。原因与替代方案见 `doc/design/modules/core/去多租户化-概要设计.md`。
...
本文档是车联网平台 **gentry-core 模块全局基础设施** 的架构设计与开发规范。它既是架构设计文档（描述系统结构、组件关系、数据流向），也是开发规范（定义编码约束、命名规则、使用方式），所有业务模块开发必须遵守本文档定义的规范。
...
这些缺陷的根因是 **gentry-core 模块缺少全局基础设施组件**。本文档定义的 10 个组件将系统性解决这些问题，并为所有业务模块提供统一的技术基座。
...
graph TB
    subgraph P0["P0 - 核心基础设施"]
        ...
        TEN["多租户拦截器<br/>GentryTenantManager"]
...
所有全局基础设施代码位于 `gentry-core` 模块，按功能领域组织：
...
│   # StpInterfaceImpl 位于 gentry-business 模块：
│   # com.gentry.rbac.security.StpInterfaceImpl
│
├── tenant/                          # 多租户
│   ├── GentryTenantManager.java  # TenantFactory 实现
...
└── web/                             # Web 通用组件
    ...
    └── RequestLogProperties.java    # gentry.request-log.* 配置属性
```
...
| 多租户拦截器 | [P0-多租户拦截器-概要设计.md](../modules/core/P0-多租户拦截器-概要设计.md) | [P0-多租户拦截器-详细设计.md](../modules/core/P0-多租户拦截器-详细设计.md) | `GentryTenantManager`、`@IgnoreTenant` |
...
| StpInterface 权限接口 | [P0-StpInterface权限接口-概要设计.md](../modules/core/P0-StpInterface权限接口-概要设计.md) | [P0-StpInterface权限接口-详细设计.md](../modules/core/P0-StpInterface权限接口-详细设计.md) | `StpInterfaceImpl`（位于 gentry-business） |
...
**实现方案（实际代码）：**
```java
// 在 gentry-core 模块的 SaTokenConfig 中配置
@Configuration
...
Phase 3: 多租户拦截器
    ↓
Phase 4: StpInterface 权限接口（依赖 Mapper，在 gentry-business）
    ↓
...
gentry-core 模块需要添加以下依赖：

```xml
<!-- backend/gentry-core/pom.xml -->
<dependencies>
...
| GentryTenantManager | 90% | tenant_id 注入、忽略表、@IgnoreTenant |
