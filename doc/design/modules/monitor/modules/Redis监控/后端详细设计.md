...
> `SUPER_ADMIN`」权限层级描述已不适用（`SUPER_ADMIN` 已并入 `ADMIN`，`ADMIN` 现为
> 唯一角色并持有全部 Redis 监控权限）。Redis 监控本身的接口设计仍然有效。
...
- 该操作不可恢复，仅 `monitor:redis:slowlog:reset` 权限（去多租户化后为 `ADMIN`，见文末过期说明）可调用
- 前端需二次确认
...
| API-007 | `monitor:redis:slowlog:reset` | 清空慢日志 | 仅 ADMIN（原文写 SUPER_ADMIN，已并入 ADMIN，见文末过期说明） |
