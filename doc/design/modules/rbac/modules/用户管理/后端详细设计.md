...
| post_name | VARCHAR(50) | 否 | 职务名称 | 职务标签 | 从字典 `sys_user_post` 选择，存储字典 label 值 |
...
|---|------|---------|---------|-----------|------|
| sys_user | post_name | sys_dict_data.dict_label | 避免列表查询时联查字典表 | 设置时直接存值，字典变更不影响已存数据 | 低（职务标签变更频率低） |
...
| postName | postName | 直接映射 | 存字典 label 值 |
...
```json
{
  ...
  "data": {
    ...
    "userInfo": {
      ...
      "deptName": "总公司",
      "roles": [
        { "id": 1, "roleCode": "ADMIN", "roleName": "管理员", "dataScope": 1 }
      ],
