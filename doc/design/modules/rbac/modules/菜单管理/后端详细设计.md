> **⚠️ 部分内容已过期（2026-09-01）**：本仓库已拿掉多租户机制，本文中「不受租户拦截
> 器过滤」「菜单数据对所有租户共享」等描述已不适用（多租户拦截器整个已删除，`sys_menu`
> 一直就是全局表，这个定位没变，只是"全局表 vs 租户表"这个分类维度本身消失了）。
> 菜单管理本身的字段/接口/流程设计仍然有效。
