> **⚠️ 部分内容已过期（2026-09-01）**：本仓库已拿掉多租户机制，本文中所有「多租户
> 拦截器 / `TenantInterceptor` / `TenantContext` / `TenantEntity` / `tenant_id` 自动
> 追加」相关描述均已不适用（组件已删除，实体统一继承 `BaseEntity`）。错误码表里的
> `TENANT_NOT_FOUND` 等 4 个租户错误码已删除且号段不回收复用。其余章节（异常处理、
> 自动填充、数据权限、认证鉴权、操作日志）仍然有效。
