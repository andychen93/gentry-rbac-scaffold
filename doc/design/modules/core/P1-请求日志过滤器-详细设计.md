...
> **实现状态：✅ 已实现** — 代码位于 `gentry-core/src/main/java/com/gentry/core/web/RequestLogFilter.java`，覆盖 8 个单元测试 + 端到端验证。
...
**包路径**：`com.gentry.core.web.RequestLogFilter`
...
**包路径**：`com.gentry.core.web.RequestLogProperties`

**注解**：`@ConfigurationProperties(prefix = "gentry.request-log")`
...
**包路径**：`com.gentry.core.web.RequestLogFilterConfig`
...
| 配置项 | 键 | 默认值 | 说明 |
|--------|-----|--------|------|
| 启用开关 | `gentry.request-log.enabled` | `true` | 生产环境可按需关闭 |
| 慢请求阈值 | `gentry.request-log.slow-threshold` | `500` | 单位毫秒 |
| 排除路径 | `gentry.request-log.exclude-paths` | `[/actuator/**, ...]` | Ant 风格路径模式 |
| 最大请求体长度 | `gentry.request-log.max-body-length` | `2048` | 超过截断 |
...
**测试类**：`com.gentry.core.web.RequestLogFilterTest`
...
**测试类**：`com.gentry.core.web.RequestLogFilterIntegrationTest`
