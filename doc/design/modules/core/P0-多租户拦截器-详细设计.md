...
| 项目 | 内容 |
|------|------|
| 组件名称 | 多租户拦截器（PrecisionTenantManager） |
| 版本 | v1.0.0 |
| 日期 | 2026-04-12 |
| 所属模块 | precision-core |
...
classDiagram
    ...
    class TenantConfig {
        <<@Configuration>>
        +mybatisFlexTenantManager() PrecisionTenantManager
    }
    ...
    TenantFactory <|.. PrecisionTenantManager : 实现
    PrecisionTenantManager ..> UserContext : 获取 tenantId
    PrecisionTenantManager ..> TenantConstants : 读取默认租户
    TenantConfig ..> PrecisionTenantManager : 注册 Bean
    IgnoreTenant ..> PrecisionTenantManager : 标记跳过
```

## 2.2 核心类详细设计

### 2.2.1 PrecisionTenantManager

**包路径：** `com.precision.core.tenant.PrecisionTenantManager`

**职责：** 实现 MyBatis-Flex `TenantFactory` 接口，为所有租户表的 SQL 自动注入 `tenant_id` 条件。

**实现方式：** MyBatis-Flex 1.11.6 中，`TenantFactory` 是标准 SPI 接口，提供 `getTenantIds()` 方法返回当前租户 ID 数组。`PrecisionTenantManager` 需注册为 Spring Bean 并在 `MybatisFlexCustomizer` 中通过 `TenantManager.setTenantFactory()` 配置。
...
**实现位置：** `PrecisionTenantManager.getIgnoreTables()`
...
|------|---------|---------|---------|--------|
| ignoreTables() | 全局 | PrecisionTenantManager | 无tenant_id的全局表 | 最高 |
...
**包路径：** `com.precision.core.config.TenantConfig`

**职责：** Spring 配置类，注册 `PrecisionTenantManager` 和 `IgnoreTenantAspect` 为 Spring Bean。
...
public class MyBatisFlexConfig {
    ...
    @Autowired
    private PrecisionTenantManager tenantManager;
...
### PrecisionTenantManager 与 MyBatis-Flex 的交互
...
sequenceDiagram
    ...
    participant TM as PrecisionTenantManager
...
<!-- mybatis-flex（需在 precision-core 或 precision-infrastructure 中引入） -->
<dependency>
...
**测试类：** `com.precision.core.interceptor.PrecisionTenantManagerTest`
...
**测试类：** `com.precision.core.interceptor.IgnoreTenantAspectTest`
...
**测试类：** `com.precision.core.interceptor.TenantInterceptorIntegrationTest`
