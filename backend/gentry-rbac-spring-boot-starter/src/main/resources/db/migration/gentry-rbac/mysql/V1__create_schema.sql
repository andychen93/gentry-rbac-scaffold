-- =============================================
-- RBAC 权限脚手架 - 建表（MySQL / MariaDB 8.0+）
-- =============================================
-- 本文件与 postgresql/V1、sqlite/V1 是同一份表结构的三份方言实现，
...
CREATE TABLE IF NOT EXISTS sys_tenant (
    ...
    remark          VARCHAR(500),
    -- 租户扩展配置：Java 侧是裸 String（Service 层自行 JSON 解析），未用 MySQL JSON 类型的
    -- 校验/函数特性，用 TEXT 保持三库一致，避免引入方言依赖
    config          TEXT,
    ...
    create_by       BIGINT,
    update_by       BIGINT,
    -- MySQL 不支持局部索引（WHERE 子句），把 deleted 纳入唯一键组合，
    -- 效果等价于 PostgreSQL/SQLite 版本的 `WHERE deleted = 0` 局部唯一索引
...
CREATE TABLE IF NOT EXISTS sys_user (
    ...
    create_by       BIGINT,
    update_by       BIGINT,
    UNIQUE KEY uk_user_tenant_username (tenant_id, username, deleted),
    -- 手机号允许为空；phone 入唯一键后 MySQL 对 NULL 不做唯一性约束（同 PG/SQLite），
    -- 多个 NULL 不冲突，效果等价于原 PG 版本 `WHERE phone IS NOT NULL AND deleted = 0`
    UNIQUE KEY uk_user_tenant_phone (tenant_id, phone, deleted),
...
CREATE TABLE IF NOT EXISTS sys_oper_log (
    ...
    KEY idx_oper_log_tenant_time (tenant_id, operate_time)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
