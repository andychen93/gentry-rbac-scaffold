-- =============================================
-- V19 拿掉多租户 · 第三步：删 tenant_id 列（SQLite 方言）
--
-- 详见 doc/design/modules/core/去多租户化-概要设计.md
--
-- 与 mysql/V19、postgresql/V19 是同一份改动的三份方言实现，字段和索引语义必须
-- 保持一致。写法与 postgresql/V19 相同（局部唯一索引 SQLite 原生支持）。
--
-- 注意：SQLite 3.35+ 才支持 ALTER TABLE ... DROP COLUMN（本仓库最低版本要求
-- 已是 3.35+，见 database-migration 约定），旧版本需要建临时表迁数据，此处不考虑。
-- =============================================
