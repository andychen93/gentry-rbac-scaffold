 ...
 * 真正生效的一直是 yml 里的 {@code gentry.login-security.*} 等属性。
 ...
 *   <tr><td>sys.login.maxFailCount</td><td>gentry.login-security.max-fail-count</td></tr>
 *   <tr><td>sys.login.lockMinutes</td><td>gentry.login-security.lock-minutes</td></tr>
 *   <tr><td>sys.password.expireDays</td><td>gentry.password.expire-days</td></tr>
 *   <tr><td>sys.captcha.enabled</td><td>gentry.captcha.enabled</td></tr>
 * </table>
 *
 * <h3>关闭 DB 覆盖</h3>
 * 设 {@code gentry.config.db-override=false} 可让本类始终返回 yml 默认值，
...
public class SysConfigResolver {
    ...
    public SysConfigResolver(ConfigService configService,
                             @Value("${gentry.config.db-override:true}") boolean dbOverride) {
