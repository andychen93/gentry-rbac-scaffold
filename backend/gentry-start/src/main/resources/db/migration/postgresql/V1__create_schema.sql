...
-- =============================================
-- 本文件与 mysql/V1、sqlite/V1 是同一份表结构的三份方言实现，
-- 字段和索引语义必须保持一致。改了这份记得同步另外两份，
-- 或者看 .kiro/steering/database-migration.md 里"何时需要分厂商目录"的判断标准。
--
-- 相比 mysql/V1：局部唯一索引（WHERE deleted = 0）是 PostgreSQL 原生能力，
-- 不需要像 MySQL 那样把 deleted 拼进组合唯一键。
...
CREATE TABLE IF NOT EXISTS sys_tenant (
    ...
    remark          VARCHAR(500),
    -- 租户扩展配置：Java 侧是裸 String（Service 层自行 JSON 解析），未用 JSONB 的
    -- 查询/索引特性，用 TEXT 保持三库一致，避免引入方言依赖
    config          TEXT,
