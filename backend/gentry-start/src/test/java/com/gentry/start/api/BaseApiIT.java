 ...
 * 完整经过 Sa-Token 鉴权 / 权限 / 数据权限链路。
 *
 * <p>鉴权：通过<b>真实 {@code /auth/login}</b> 取 token。登录流程会往 SaSession 写入
 * {@code userId/deptId/permissionList}（见 AuthServiceImpl.buildLoginResult），
 * SaTokenConfig 拦截器据此重建 UserContext。
 *
 ...
 * <ul>
 *   <li>{@code admin} = ADMIN(role_id=1)，管理员，拥有全部菜单，跑绝大多数 happy-path。</li>
...
public abstract class BaseApiIT {
    ...
    protected static final String ADMIN = "admin";          // ADMIN(role_id=1)，拥有全部菜单
